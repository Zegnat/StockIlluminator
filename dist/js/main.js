    ...
    var MarketSize = 75; /* How many corps? */
    var CoinFlips = 50;  /* How many coin flips per interval */
    var GameSpeed = 10; /* seconds */
    var PlayerName = ''; /* The players starting name */
    if($.cookie('PlayerName') && $.cookie('PlayerName') != '' )PlayerName = $.cookie('PlayerName');
    var PlayerCash = 10000; /* The players starting cash */
    if($.cookie('PlayerCash'))PlayerCash = $.cookie('PlayerCash');
    var TradeAmount = 10; /* The starting buy amount */
	TradeAmount = $('#TradeAmount').val();
    var MaxDebt = -10000; /* Max debt the player is allowed before gameover */
	var numAnimTime = 0.35; /* Animation time on numbers */
	...
	if($.cookie('OwnedItems'))OwnedItems = ($.cookie('OwnedItems')).split('.');
    ...
    var CoSrc = [' ',' ','.gov ','.io ', ' group',' incorporated',' corp',' industries','.com '];
    ...
    if(this.value!=='') MarketSize = this.value;
    GenMarket();
    });
    $('#CoinFlips').on('keyup change', function(){
    if(this.value!=='') CoinFlips = this.value;
    });
    $('#GameSpeed').on('keyup change', function(){
    if(this.value!=='') {GameSpeed = this.value;
						FlipCount=GameSpeed;}
	});
	
    $(document.body).on('click', '.BuyLink', function() {
    var ind = $(this).data("index");
    BuyStock(ind, parseFloat(TradeAmount));
    UpdateHUD();
    });   

    $(document.body).on('click', '.SellLink', function() {
    var ind = $(this).data("index");
    SellStock(ind, parseFloat(TradeAmount));
    UpdateHUD();
    });
                   
    $(document.body).on('click', '.ItemBuyLink', function() {
    var ind = $(this).data("index");
    BuyItem(ind);
    });
    ...
    for (var i=0;i<MarketSize;i++)
    {
	var Adj = RandomVal(AdjSrc); var Noun = RandomVal(NounSrc); var Co = RandomVal(CoSrc);
    var Name = Adj.value + ' ' + Noun.value + Co.value;
    Corps.push({id: cuniq(),
              name: Name,
    ...
    function FlipCoin(Coin, Bias, Flips) { /* Yup, that simple */
    var Max = 1 + Bias;
    ...
    /* Get New Bias */
    if(Obj.last1 >= last)  { Bias = Bias - 0.05; }      /* Past higher than now */
    if(Obj.last2 >= Obj.last1)  { Bias = Bias - 0.05; } /* Lower expectations */
    if(Obj.last3 >= Obj.last2)  { Bias = Bias - 0.05; }
    if(Obj.last1 <= last) { Bias = Bias + 0.05; }      /* Past lower than now */
    if(Obj.last2 <= Obj.last1) { Bias = Bias + 0.05; } /* Increase expectations */
    if(Obj.last3 <= Obj.last2) { Bias = Bias + 0.05; }
    ...
    var cuniq_d = new Date(),
        ...
        cuniq_u = ++cuniq_d + cuniq_m + (++cuniq_c === 10000 ? (cuniq_c = 1) : cuniq_c);
    ...
    OwnedStocks[ind].amount = amount;} else {
	...
	if(!isNumber(i[a])) { Output += base.indexOf(i[a]);
	} else {
	...
	return (OwnedItems.length-1)+'/'+itoa(String(PlayerCash).replace('-','0'))+'/'+encodeURIComponent(PlayerName) + '/';
	}
	
	function ShowProfile(para) {
	var Arr = para.split('/');
	var LCash = itoa(Arr[1]);
	LCash = LCash.indexOf('0') == 0 ? '-'+LCash.substring(1) : LCash;
	$('.modal-body').html('<h2>'+decodeURI(Arr[2]) + '</h2><br><br><h3>' + StoreItems[Arr[0]].desc +'</h3><br><br><h3>BANKED: '+'$'+ numberWithCommas(LCash) + '</h3><br><br><h3>Tel: 311-555-2368</h3>');
    ...
    var ThirdLetter = Words[Words.length - 2].charAt(0);
    ...
    if(OwnedStocks[ind].amount > 0){
	XClass = ' owned';
    Tooltip = 'data-toggle="hover" data-placement="top" title="" data-original-title="'+
                'Profit per stock: $'+AvgPriceDiff(ind) + '" ';
    Cash = '$'+CashInMarket(ind)+' ('+StocksOwned(ind)+')';           
    }}

    var Dir = '<span class="glyphicon glyphicon-minus"><\/span>';
    if(Obj.dir == '/\\') { XClass = 'success'+XClass; Dir= '<span class="glyphicon glyphicon-chevron-up"><\/span>';}
    if(Obj.dir == '\\/') { XClass = 'danger'+XClass; Dir= '<span class="glyphicon glyphicon-chevron-down"><\/span>';}
	
	var sellButton = '<button type="button" data-index="'+ind+'" '+Tooltip+'class="SellLink btn btn-sm btn-default">Sell<\/button>';
	var content = '<tr id="'+Obj.id+'_row" class="'+XClass+'"><td>'+Obj.ticker+'<\/td><td>'+Obj.name+'<\/td><td>$'+Obj.point+'<\/td><td class="CashInMarket">'+Cash+'<\/td><td>'+Dir+' '+GetDiff(Obj.point,Obj.last2)+'<\/td><td><button type="button" data-index="'+ind+'" class="BuyLink btn btn-sm btn-info">Buy<\/button><\/td><td>'+sellButton+'<\/td><\/tr>';
	...
	$('#'+Obj.id+'_row').find( "td" ).eq( 4 ).html(Dir+' '+GetDiff(Obj.point,Obj.last2));
	...
	if(Change == 0){
	var numAnim = new countUp("PlayerCash", PlayerCash, 0, numAnimTime);
	numAnim.start();
	}
    $('#PlayerCash').html(numberWithCommas(PlayerCash)); //As a backup incase above fails.
	
	if(InMarket != CashInMarket('all')){
	numAnim = new countUp("CashInMarket", CashInMarket('all'), 0, numAnimTime);
	numAnim.start();
    ...
    var XClass = '';
    var Button = '<button type="button" data-index="'+ind+'" class="ItemBuyLink btn btn-sm btn-info">Buy<\/button>';
    ...
    var content = '<tr id="item_'+ind+'_row" class="'+XClass+'"><td>'+Obj.name+'<\/td><td>'+'$'+numberWithCommas(Obj.cost)+'<\/td><td>'+Button+'<\/td><\/tr>';
    ...
    /* All together now.. */
	if(getURLParameter('p')){ShowProfile(getURLParameter('p'));} //Show any profiles linked to
    $('.tooltipTog').tooltip();
    GenMarket();
    GenStore();
    FlipInterval();
